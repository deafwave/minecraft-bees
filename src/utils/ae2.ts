...
export function transferItemFromAE2(
	itemId: string,
	targetPeripheralType: string,
	minCount: number,
	maxCount: number,
	targetSlot: number,
): void {
	const interfacePeripheral = peripheral.find<IGrid>(
	...
	const unknownPeripheral =
		peripheral.find<InventoryPeripheral>(targetPeripheralType)
...
export function findOrCraftItem(
	itemId: string,
	count: number,
): IAEItemStack | undefined {
	const interfacePeripheral = peripheral.find(
		'appliedenergistics2:interface',
	) as unknown as IGrid
