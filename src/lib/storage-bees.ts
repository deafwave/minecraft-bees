...
interface ExistingBees {
	...
	princesses: {
		[species: string]: { storage: InventoryPeripheral; slot: number }
	}
	drones: {
		[species: string]: { storage: InventoryPeripheral; slot: number }
	}
...
function getBeeStorages(
	includeMutatrons: boolean = true,
	includeApiaries: boolean = true,
): InventoryPeripheral[] {
	const crates = peripheral.find<InventoryPeripheral>(
		'actuallyadditions:giantchestlarge',
	)
	let apiaries = includeApiaries
		? peripheral.find<InventoryPeripheral>('gendustry:industrial_apiary')
		: []
	let mutatrons = includeMutatrons
		? peripheral.find<InventoryPeripheral>('gendustry:mutatron')
		: []

	return [...crates, ...apiaries, ...mutatrons]
}

export function getExistingBees(
	includeMutatrons: boolean,
	includeApiaries: boolean,
): ExistingBees {
	...
	storages.forEach((storage) => {
		...
		if (listResult) {
			Object.entries(listResult).forEach(([slot, item]) => {
				if (
					item.name === 'forestry:bee_princess_ge' ||
